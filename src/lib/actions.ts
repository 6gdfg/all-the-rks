...
export async function createClassAction(formData: FormData) {
  ...
  const autoClassFirst = readCheckbox(formData, "autoClassFirst");
  ...
  await sql`
    INSERT INTO class_settings (class_id, auto_class_first)
    VALUES (${classId}, ${autoClassFirst})
...
export async function saveScoresAction(formData: FormData) {
  ...
  `;
  const settingsRows = await sql<{ autoClassFirst: boolean }[]>`
    SELECT auto_class_first AS "autoClassFirst"
    FROM class_settings
    WHERE class_id = ${classId}
    LIMIT 1
  `;
  ...
  const autoClassFirst = settingsRows[0]?.autoClassFirst ?? true;
  const manualFirstByExam = new Map<number, number>();
  ...
  if (!autoClassFirst) {
    for (const exam of exams) {
      ...
      const studentId = Number(formData.get(`manualFirst_${examId}`));

      if (studentIds.has(studentId)) {
        manualFirstByExam.set(examId, studentId);
      }
    }
  }
  ...
  for (const [key, value] of formData.entries()) {
    ...
    changes.push({
      ...
      examId,
      score,
      isManualClassFirst: manualFirstByExam.get(examId) === studentId
    });
  }
        ...
        if (autoClassFirst) {
          ...
          await tx`
            ...
            VALUES (
              ...
              ${change.examId},
              ${change.score},
              ${change.isManualClassFirst},
              NOW()
            )
            ON CONFLICT (student_id, exam_id) DO UPDATE
            SET score = EXCLUDED.score,
                is_manual_class_first = EXCLUDED.is_manual_class_first,
                updated_at = NOW()
          `;
        }
      }
...
export async function updateClassInlineAction(
...
export async function createClassInlineAction(
  _state: InlineActionState,
  formData: FormData
) {
  return runInlineAction(async () => {
...
export async function updateSettingsInlineAction(
  _state: InlineActionState,
  formData: FormData
) {
  return runInlineAction(async () => {
    await saveClassSettings(formData);
    return "展示设置已保存。";
  });
}

export async function createStudentInlineAction(
  _state: InlineActionState,
  formData: FormData
) {
  return runInlineAction(async () => {
    return createStudentWithoutRedirect(formData);
  });
}

export async function studentRowInlineAction(
  _state: InlineActionState,
  formData: FormData
) {
  return runInlineAction(async () => {
    if (readText(formData, "intent", 20) === "delete") {
      await deleteStudentWithoutRedirect(formData);
      return "学生已删除。";
    }

    await updateStudentWithoutRedirect(formData);
    return "学生信息已保存。";
  });
}

export async function createExamInlineAction(
  _state: InlineActionState,
  formData: FormData
) {
  return runInlineAction(async () => {
    await createExamWithoutRedirect(formData);
    return "考试已创建。";
  });
}

export async function examRowInlineAction(
  _state: InlineActionState,
  formData: FormData
) {
  return runInlineAction(async () => {
    if (readText(formData, "intent", 20) === "delete") {
      await deleteExamWithoutRedirect(formData);
      return "考试已删除。";
    }

    await updateExamWithoutRedirect(formData);
    return "考试信息已保存。";
  });
}

export async function saveScoresInlineAction(
  _state: InlineActionState,
  formData: FormData
) {
  return runInlineAction(async () => {
    await saveScoresWithoutRedirect(formData);
    return "成绩已保存，RKS 已自动刷新。";
  });
}
...
async function updateStudentWithoutRedirect(formData: FormData) {
  const teacher = await requireTeacher();
  const classId = readId(formData, "classId");
  const studentId = readId(formData, "studentId");
  const name = readText(formData, "name", 60);
...
async function createExamWithoutRedirect(formData: FormData) {
  ...
  if (!name || totalScore <= 0 || constantValue < 0) {
    throw new Error("请填写有效的考试名称、总分和定数。");
  }
...
async function saveScoresWithoutRedirect(formData: FormData) {
  ...
  for (const [key, value] of formData.entries()) {
    ...
    if (
      ...
      throw new Error("成绩必须是 0 到考试总分之间的数字。");
    }
